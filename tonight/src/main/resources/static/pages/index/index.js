...
App.start({
    ...
    data: function () {
        var validatePass = (rule, value, callback) => {
            if (value === '') {
                ...
                if (this.userPassword.newPassword !== '') {
                    this.$refs.userPassword.validateField('checkPassword');
                }
                callback();
            }
        };
        ...
        return {
            ...
            updateUserNameDialogFormVisible: false,
    ...
    mounted() {
         ...
         * 获取用户名
         */
        App.request("/user/getUsername").callSuccess(function (resp) {
    ...
    methods: {
        handleCommand(command) {
            if ("updateUsername" === command) {
                this.userName.username = this.username;
                this.updateUserNameDialogFormVisible = true;
            } else if ("updatePassword" === command) {
                this.updatePasswordDialogFormVisible = true;
            }
            else if ("logout" === command) {
                App.request("/logout").callSuccess(function (resp) {
                    if (resp.data) {
        ...
        /*修改用户名的确认按钮*/
        updateUserNameDialogFormVisibleSubmit(userName) {
            let that = this;
            this.$refs[userName].validate((valid) => {
                if (valid) {
                    App.request("/user/registerVerify", {username: that.userName.username}).callSuccess(function (resp) {
                        if (resp.data) {
                            App.request("/user/updateUsername", {
                                "oldUsername": that.username,
                                "newUsername": that.userName.username
                            }).callSuccess(function (resp) {
                                if (resp.data) {
                                    that.$message({
                                        type: 'success',
                                        message: '修改用户名成功!'
                                    });
                                    that.username = that.userName.username;
                                    that.updateUserNameDialogFormVisible = false;
                                } else {
                                    that.$message({
                                        type: 'error',
                                        message: '修改用户名失败!'
        ...
        updatePasswordDialogFormVisibleSubmit(userPassword){
            let that = this;
            this.$refs[userPassword].validate((valid) => {
                if (valid) {
                    App.request("/user/loginVerify").post().setData({"username":that.username,"password":that.userPassword.oldPassword}).callSuccess(function (resp) {
                        if (resp.data){
                            App.request("/user/updatePassword",{"username":that.username,"newPassword":that.userPassword.newPassword}).callSuccess(function (resp) {
                                if (resp.data) {
                                    that.$message({
                                        type: 'success',
                                        message: '修改密码成功!'
                                    });
                                    that.updatePasswordDialogFormVisible = false;
                                } else {
                                    that.$message({
                                        type: 'error',
                                        message: '修改密码失败!'
...
//添加页面子模块导航
        ...
        addPageTag: function (menuId, url_) {
            ...
            //设置标题
            document.title = menuObj.name == '首页' ? 'API管理系统' : menuObj.name;
        ...
        closeNav(item) {
            ...
            var compId = this.convertToComp(window.location.hash.substring(1));
            var index = this.cacheList.indexOf(compId);
            if (index != -1) {
                this.$delete(this.cacheList, index);
            }
