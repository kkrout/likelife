        ...
        methods: {
           ...
           /*     this.form.className='';*/
            ...
            handleClick: function (row) {
                App.putData("row", row);
                App.openModule("updateAPI", "修改API", "api/update/update.html");
            },
            /*表格类别显示状态转换*/
            classes(val) {
                let that = this;
                for (let i = 0; i < that.options.length; i++) {
                    if ((that.options[i].value) === val.classes) {
                        return that.options[i].label;
                    }
                }
                return '未匹配类型';
            },
            ...
            nodeClick(data, Node) {
                ...
                if (Node.level===2) {
                    ...
                    App.request("/api/findApiClassList",{classes:data.value}).callSuccess(function (resp) {
                        for (let i = 0; i < that.options.length; i++) {
                            if (that.options[i].value == data.value) {
                                that.$set(that.options[i], 'children', resp.data)
                            }
                        }
                    });
                }*/
